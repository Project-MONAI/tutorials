...
def main():
    ...
    train_transforms = Compose(
        [
            ...
            RandRotated(
                keys=["image", "label"], range_x=np.pi, prob=0.8, mode=["bilinear", "nearest"]
            ),
            RandZoomd(
                keys=["image", "label"],
                min_zoom=0.8,
    ...
    for active_iter in range(args.active_iters):
        ...
        print("Training Finished, Test Data Score in IoU: ", iou, time.strftime("%Y-%m-%d, %H:%M:%S", time.localtime(time.time())))
