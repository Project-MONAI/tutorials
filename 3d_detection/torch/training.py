...
def main():
    ...
    for epoch in range(max_epochs):
        ...
        for batch_data in train_loader:
            ...
            targets = [
                dict(
                    labels=batch_data_i["label"].to(device),
                    boxes=batch_data_i["box"].to(device),
                )
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                for val_data in val_loader:
                    val_step += 1
                    val_inputs = [
                        val_data_i["image"].to(device) for val_data_i in val_data
                    ]
                    val_targets = [
                        dict(
                            labels=val_data_i["label"].to(device),
                            boxes=val_data_i["box"].to(device),
                        )
                        for val_data_i in val_data
                    ]
                    val_outputs = model(val_inputs, targets)
                    val_loss = (
                        val_outputs["classification"] + val_outputs["bbox_regression"]
                    )
                ...
                if val_epoch_loss < best_val_epoch_loss:
                    ...
                    torch.save(model.state_dict(), envDict["model_path"])
