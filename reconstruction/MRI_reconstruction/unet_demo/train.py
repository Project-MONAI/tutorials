...
def trainer(args):
    ...
    ##################################################### temp: dummy file
    val_files = list(
        filter(lambda f: "file_brain_AXT2_201_2010294.h5" not in str(f), val_files)
    )
    ##################################################### temp: dummy file
    ...
    train_transforms = Compose(
        [
            ...
            MaskTransform,
            ReferenceBasedSpatialCropd(
                keys=["kspace_masked_ifft"], ref_key="reconstruction_rss"
            ),
            ReferenceBasedNormalizeIntensityd(
                keys=["kspace_masked_ifft", "reconstruction_rss"],
    ...
    model = BasicUNet(
        spatial_dims=2, in_channels=1, out_channels=1, features=args.features,
    ).to(device)
