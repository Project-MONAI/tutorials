...
from ...blocks.feature_pyramid_network import LastLevelP6P7
...
class RetinaNetHead(nn.Module):
    ...
    def compute_loss(self, targets, head_outputs, anchors, matched_idxs, debug = False,  fg_bg_sampler= None):
        ...
        return {
            "classification": self.classification_head.compute_loss(targets, head_outputs, matched_idxs, debug = debug,  fg_bg_sampler= fg_bg_sampler),
...
class RetinaNetRegressionHead(nn.Module):
    ...
    def compute_loss(self, targets, head_outputs, anchors, matched_idxs):
        ...
        for targets_per_image, bbox_regression_per_image, anchors_per_image, matched_idxs_per_image in zip(
            ...
            # compute the loss
            losses.append(
                torch.nn.functional.smooth_l1_loss(bbox_regression_per_image, target_regression, beta=1 / 9,reduction="mean")
            )
...
class RetinaNet(nn.Module):
    ...
    def compute_loss(self, targets, head_outputs, anchors):
        ...
        for anchors_per_image, targets_per_image in zip(anchors, targets):
            ...
            if self.debug:
                print(f"Max IoU between anchors and gt boxes: {torch.max(match_quality_matrix)}. Please adjust base_anchor_size and base_aspect_ratios if too small.")
            matched_idxs.append(self.proposal_matcher(match_quality_matrix))

        return self.head.compute_loss(targets, head_outputs, anchors, matched_idxs, debug = self.debug,  fg_bg_sampler=self.fg_bg_sampler)
    ...
    def postprocess_detections(self, head_outputs, anchors, image_shapes):
        ...
        for index in range(num_images):
            ...
            for c in range(self.num_classes):
                ...
                keep_c = box_ops.non_max_suppression(image_boxes[image_labels_c_idx,:], image_scores[image_labels_c_idx], self.nms_thresh)
    ...
    def forward(self, images, targets=None):
        ...
        if self.training:
            ...
            detections = self.transform.postprocess(detections, [img.shape[-self.spatial_dims:] for img in images], image_sizes)
...
def retinanet_resnet50_fpn(
    spatial_dims: int = 3, pretrained=False, progress=True, num_classes=2, pretrained_backbone=True, trainable_backbone_layers=None, **kwargs
):
    """
    ...
    # If 2D, directly use torchvision functions
    if spatial_dims==2:
        return torchvision.models.detection.retinanet_resnet50_fpn(pretrained=pretrained, progress=progress, num_classes=num_classes, pretrained_backbone=pretrained_backbone, trainable_backbone_layers=trainable_backbone_layers, **kwargs)
