...
start_fl() {
  ...
  lst=$SECONDS
((python3 -u -m nvflare.private.fed.app.client.client_train -m $DIR/.. -s fed_client.json --set secure_train=true multi_gpu=false uid=client3 config_folder=config 2>&1 & echo $! >&3 ) 3>$DIR/../pid.fl | tee -a $DIR/../log.txt &)
  pid=`cat $DIR/../pid.fl`
  echo "new pid ${pid}"
}
