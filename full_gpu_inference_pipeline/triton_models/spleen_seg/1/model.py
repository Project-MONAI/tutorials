...
import json
import logging
from monai.transforms.utility.array import ToDevice
import numpy as np
import os
import pathlib
...
from monai.transforms import ToCupy, ToTensor
...
class TritonPythonModel:
    ...
    def initialize(self, args):
        ...
        output0_config = pb_utils.get_output_config_by_name(
            self.model_config, "MASK")

        # Get OUTPUT1 configuration
        output1_config = pb_utils.get_output_config_by_name(
            self.model_config, "CONTOUR")
        ...
        if(self.inference_device_kind == 'GPU'):
            # print("*********************I'm here********************8")
            infer_transforms.append(
            EnsureType(device=torch.device(f"cuda:{self.inference_device_id}"))
            )
        else:
            infer_transforms.append(
            EnsureType()
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            inference_request = pb_utils.InferenceRequest(
                model_name="segmentation_3d",
                requested_output_names=["OUTPUT__0"],
                inputs=[transform_tensor])
