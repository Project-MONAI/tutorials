...
from monai.apps.reconstruction.networks.nets.coil_sensitivity_model import (
...
from monai.transforms import Compose, EnsureTyped, LoadImaged, SpatialCrop
...
def trainer(args):
    ...
    train_transforms = Compose(
        [
            ...
            ExtractDataKeyFromMetaKeyd(
                keys=["reconstruction_rss", "mask"], meta_key="kspace_meta_dict"
            ),
            MaskTransform,
    ...
    val_loader = DataLoader(
        val_ds, batch_size=1, shuffle=True, num_workers=args.num_workers
    )

    # create the model
    coil_sens_model = CoilSensitivityModel(
        spatial_dims=2, features=args.sensitivity_model_features
    )
    refinement_model = ComplexUnet(spatial_dims=2, features=args.features)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = VariationalNetworkModel(
        coil_sens_model, refinement_model, num_cascades=args.num_cascades
    ).to(device)
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_data in train_loader:
            ...
            for i in range(0, num_slices, args.batch_size):
                stop_slice = min(i + args.batch_size, num_slices)
                inp = input[:, i:stop_slice, ...]
                tar = target[:, i:stop_slice, ...]
            ...
            with torch.no_grad():
                ...
                for val_data in val_loader:
                    ...
                    for i in range(0, num_slices, args.batch_size):
                        ...
                        roi_center = tuple(i // 2 for i in output.shape[-2:])
                        cropper = SpatialCrop(
                            roi_center=roi_center, roi_size=final_shape
                        )
                        output_crp = cropper(output).unsqueeze(0)
                ...
                if metric > best_metric:
                    ...
                    torch.save(
                        model.state_dict(),
                        os.path.join(outpath, "varnet_mri_reconstruction.pt"),
                    )
...
def __main__():
    ...
    parser.add_argument(
        "--num_cascades", default=12, type=int, help="number of cascades"
    )
