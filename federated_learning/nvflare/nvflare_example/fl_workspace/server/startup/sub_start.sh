...
start_fl() {
  ...
  lst=$SECONDS
((python3 -u -m nvflare.private.fed.app.server.server_train -m $DIR/.. -s fed_server.json --set secure_train=true config_folder=config 2>&1 & echo $! >&3 ) 3>$DIR/../pid.fl | tee -a $DIR/../log.txt &)
  pid=`cat $DIR/../pid.fl`
}
